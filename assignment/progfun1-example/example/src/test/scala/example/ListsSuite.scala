 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test(testName = "sum of negative numbers") {
    assert(sum(List(-1,-2)) === -3)
  }
  test(testName = "sum of empty list") {
    assert(sum(List()) === 0)
  }
  ...
  test(testName = "max of negative number") {
    assert(max(List(-3, -7, -10)) === -3)
  }
  test(testName = "max of empty lsit") {
    intercept[java.util.NoSuchElementException] {
      max(List())
    }
  }
  test(testName = "Maximum of duplicates") {
