...
class FunSetSuite extends FunSuite {
  ...
  trait TestSets {
    ...
    def sp(elem: Int): Boolean = elem >= 0

    def sn(elem: Int): Boolean = elem <= 0
  }
  ...
  test("intersection contains element present in the 2 the two sets") {
    new TestSets {
      val s: Set = intersect(sp, s2)
  ...
  test("intersection of disjoint set is empty") {
    new TestSets {
  ...
  test("difference of two sets") {
    new TestSets {
      val s: Set = diff(sp, s1)
      assert(!contains(s, 1), "diff 1")
      assert(contains(s, 2), "diff 2")
    }
  }

  test("filter of one set") {
    new TestSets {
      val s: Set = filter(sp, sn)
  ...
  test("map") {
    new TestSets {
      val s: Set = map(s1, (elem: Int) => 2 * elem)
      assert(!contains(s, 1))
      assert(contains(s, 2))
    }
  }
