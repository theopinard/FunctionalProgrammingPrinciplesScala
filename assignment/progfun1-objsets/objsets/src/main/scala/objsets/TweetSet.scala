...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val moreRetweeted = this.filter(tweet => tweet.retweets > elem.retweets)
...
object GoogleVsApple {
  ...
  def keywordFilter(keywords: List[String]): Tweet => Boolean = {
    tweet: Tweet => keywords.exists(key => tweet.text.contains(key))
  }

  lazy val googleTweets: TweetSet = allTweets.filter(keywordFilter(google))
  lazy val appleTweets: TweetSet = allTweets.filter(keywordFilter(apple))
